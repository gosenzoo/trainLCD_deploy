...
type IconNewPopupProps = {
    setting: settingType
    setSetting: React.Dispatch<React.SetStateAction<settingType>>
    // アイコン追加・決定時のコールバック（追加したアイコンのキーを渡す）
...
const IconNewPopup: React.FC<IconNewPopupProps> = ({
    ...
    return (
        <div className={backdropClass} onClick={onClose}>
            <div className="modal-dialog" onClick={e => e.stopPropagation()}>
                ...
                {/*
                  * 両タブを同じグリッドセルに重ねて常にレンダリングする。
                  * 非アクティブタブは visibility: hidden で不可視にしつつレイアウト上の
                  * サイズは維持するため、タブを切り替えてもポップアップサイズが変わらない。
                  * サイズは常に「設定で追加」タブ（内容量が多い方）の大きさに固定される。
                  */}
                <div className="tab-form-body" style={{display: 'grid'}}>
                    {/* 設定で追加サブタブ: 常にレンダリング、非アクティブ時は不可視 */}
                    ...
                    {/* 画像で追加サブタブ: 常にレンダリング、非アクティブ時は不可視 */}
                    <div style={{gridRow: 1, gridColumn: 1, visibility: newTab === 'image' ? 'visible' : 'hidden'}}>
                        <div className="form-row">
