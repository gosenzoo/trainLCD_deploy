...
各DrawerはテンプレートSVGを再帰的にトラバースして `lcdSVG` を組み立てる。

**`visible` 属性**

要素に `visible` 属性が付与されている場合、式を評価して `false` であればその要素（子孫含む）を出力に含めない。

| 演算子 | 説明 | 例 |
|---|---|---|
| （なし） | 変数参照。drawParams のキーを参照し、truthy なら `true` | `visible="isJapanese"` |
| `!` | 否定（変数・比較式に適用） | `visible="!isEnglish"` |
| `==` | 等価比較。左辺・右辺はそれぞれ変数または数値定数 | `visible="nowStation.lineNumberType == 0"` |
| `&&` | AND（複数条件の全成立） | `visible="isJapanese && isStation"` |
| `\|\|` | OR（いずれか成立） | `visible="isJapanese \|\| isEnglish"` |

評価は左から右へ、優先順位は `!` > `==` > `&&` > `||` の順。

- 数値定数は整数・小数いずれも記述可（例: `0`, `3`, `1.5`）
- `==` の両辺が数値に変換できる場合は数値として比較し、それ以外は文字列として比較する
- `!` は変数名だけでなく比較式全体にも適用できる（例: `!(nowStation.lineNumberType == 0)` は未対応、`!isJapanese` のみ対応）

**`lcdParts` 属性によるトラバース分岐**

| `lcdParts` 値 | 処理内容 |
|---|---|
| `static` | 要素を `cloneNode(true)` して出力に追加する |
| `textBox` | `data-param` 属性の値を drawParams のキーとしてテキストを取得し、`TextDrawer.create()` でテキスト要素を生成して出力に追加する |
| `numberingIcon` | `data-param` 属性の値を drawParams のキーとしてアイコン情報を取得し、ナンバリングアイコンを生成して出力に追加する |
| `arrangeArea` | 子要素を整列して出力に追加する |
| （なし） | `lcdParts` を持たない要素は子要素へ再帰する |

`textBox` が描画するテキスト内容は `lcdText` 属性で指定するテンプレート文字列から生成する（後述）。

**`lcdText` テンプレート属性**

`lcdText` 属性は `textBox` / `numberingIcon` 要素が描画する内容を指定するテンプレート文字列。`#{変数名}` の形式で drawParams の値を埋め込める。

```
lcdText フォーマット: 任意文字列 + #{変数名} の組み合わせ
```

| 記述例 | 展開例（drawParams: `{stationName:"渋谷", trainType:"急行", station:{eng:"Shibuya"}}`） |
|---|---|
| `"#{stationName}"` | `"渋谷"` |
| `"#{trainType}行"` | `"急行行"` |
...
`visible` 属性の変数名も同様にドット記法で入れ子オブジェクトを参照できる（例: `visible="station.isStop"`）。
...
| ファイル | 役割 |
|---|---|
| `Drawer.js` | Drawerクラス本体。同フォルダの `drawParams.json` / `iconList.json` / `headerSVG.svg` を固定パスで読み込む |
| `index.html` | テスト用HTMLページ。`TextDrawer.js` と `Drawer.js` を読み込み、描画結果を表示する |
| `drawParams.json` | テスト用描画パラメータ（boolean フラグ + テキスト文字列） |
| `iconList.json` | テスト用アイコンリスト（空オブジェクト） |
| `headerSVG.svg` | テスト用SVGテンプレート。`lcdParts` / `visible` 属性を使って記述。defs.svg に依存しない固定色で記述する |

#### Drawerクラス API

| メソッド | 説明 |
|---|---|
| `async load()` | 3ファイルを並列フェッチして初期化する |
| `draw()` | SVGをトラバースして `<g>` 要素を返す |
| `_traverse(element, parent)` | 再帰トラバース。`visible` 評価 → `lcdParts` 分岐 |
| `_evalExpr(expr)` | `visible` 式の評価（`&&` / `\|\|` / `!` 対応） |
