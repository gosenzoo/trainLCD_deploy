class DefaultLineController{
    ...
    extractDrawParams(progressParams, operation){
        ...
        for(let i = 0; i < (stationFrameNum - 1) * 2; i++){
            ...
            //中間左半分
            if(i % 2 === 0){
                colorList.push(dispStationList[Math.floor(i/2)].lineColor);
            }
            //中間右半分
            else{
                colorList.push(this.setting.stationList[dispStationList[Math.floor(i/2) + 1]._id - 1].lineColor);
            }
        }
        //最後の端の色
        colorList.push(dispStationList[dispStationList.length-1].lineColor);
        ...
        const timeList = [];
        let buf = 0;
        let nowInd = Math.floor(hereDrawPos); //現在地アイコンの直近の駅ind
        for(let i = 0; i <= nowInd; i++){
            timeList.push(-1);
        }
        for(let i = dispStationList[nowInd]._id; nowInd < stationFrameNum-1;){
        ...
        for(let i = 0; i < dispStationList.length; i++){
